...
(defun make-power-patterns2 (path)
  (loop as word in simulator::*power-words*
     as power-path in simulator::*power-moves*
     for replacement = (find-power-replacement path power-path)
     when replacement
       return (cons word replacement)
     finally (return path)))

(defun find-power-replacement (path power-path)
  (dolist (pos (find-start-positions path power-path))))

(defun find-start-positions (path power-path)
  (when-let ((end-positions (get-end-positions path (get-path-height power-path))))))

(defun get-path-height (path)
  (loop as move in path
     count (or (eql move :SE) (eql move :SW))))

(defun get-end-positions (path height)
  (unless (<= (get-path-height path) height)
    (multiple-value-bind (prefix suffix) (split-path-by-height path height)
      (let ((*curr-imagined-position* ))))))



(defun split-path-by-height (path height)
  )
